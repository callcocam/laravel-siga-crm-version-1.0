                    ...
                    <div class="vx-row no-gutter justify-center items-center">
                        ...
                        <div class="vx-col sm:w-full md:w-full lg:w-1/2 d-theme-dark-bg">
                            <div class="p-8 login-tabs-container">
                                ...
                                <div>
                                    <form ref="vuaxyForm"  @submit.prevent="loginJWT($event)" @keydown="form.onKeydown($event)">
                                        <template v-for="(item, index) in elements.rows">
                                            <SectionRenderFields :form="form" :element="setFom(item)"></SectionRenderFields>
                                        </template>
                                        <div class="flex flex-wrap justify-between my-5">
                                            <vs-checkbox v-model="form.checkbox_remember_me" class="mb-3">{{ $t('Remember Me') }}</vs-checkbox>
                                            <router-link to="">{{ $t('Forgot Password?')}}}</router-link>
                                        </div>
                                        <vs-button  type="border">{{ $t('Register') }}</vs-button>
                                        <vs-button button="submit" class="float-right"  type="filled">{{ $t('Login') }}</vs-button>
                                    </form>
                                    <vs-divider>{{ $t('OR') }}</vs-divider>
...
<script>
    ...
    export default{
        components:{
           SectionRenderFields:()=>import(/* webpackChunkName: "input-render-actions-login" */"@views/pages/form/rows/sections/SectionRenderFields"),
        },
        data() {
            return {
                form: new Form({
                    email: "leann.ullrich@example.org",
                    password: "password",
                    ...
                    rows:[
                        {
                            ...
                            sections:{
                                email: {
                                    ...
                                    icon:"icon icon-user",
                                    value:"admin@localhost.crm-03.test",
                                    attributes:{
                                        type:"email",
        ...
        methods:{
            ...
            loginJWT() {
                ...
                this.form.post("/api/v1/admin/login")
                    .then(({ data }) => {
                        this.$store.dispatch('auth/loginJWT', data)
